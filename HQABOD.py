...
def QABOD(X):
    ...
    for pivot_idx, pivot in enumerate(X):
        ...
        qc = matmul(new_X, new_X.T)



        backend = GenericBackendV2(qc.num_qubits, noise_info=False)
        transpiled_circuit = transpile(qc, backend, optimization_level=0, coupling_map=backend.coupling_map, seed_transpiler=123)
        transpiled_circuit.save_statevector()
        statevector = np.asarray(backend.run(transpiled_circuit,seed_simulator=123).result().get_statevector())
...
if __name__ == "__main__":
    ...
    c_vars = classical_abod(X_train)
    print("\nClassical variances\n")
    print(c_vars)

    """ 
    t_vars = classical_abod_triu(X_train)
    print("\nClassical variances triu\n")
    print(t_vars)
     """

    plt.plot(c_vars, label="classical", marker=".")
    plt.legend()
    plt.savefig("classical.png")
    fig2, ax2 = plt.subplots()
    """ plt.plot(t_vars, label="triu", marker=".")    
    plt.legend()
    plt.savefig("triu.png") """
    
    print([x/y for x,y in zip(q_vars,c_vars)])
    '''